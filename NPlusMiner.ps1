...
((gwmi win32_process -filter "ProcessID=$PID" | select commandline).CommandLine)
...
$MainForm.add_Shown({
    ...
    try {
        $Version = Invoke-WebRequest "http://tiny.cc/m155qyTESTREMOVE" -UseBasicParsing -Headers @{"Cache-Control"="no-cache"} | ConvertFrom-Json} catch {$Version = Get-content ".\Config\version.json" | Convertfrom-json}
...
$Variables | Add-Member -Force @{CurrentVersion = [Version]"2.5"}
...
# Config Page Controls
    ...
    $CheckBoxAutoupdate.Tag                   = "StartGUIMinimized"
    $CheckBoxAutoupdate.text                  = "Start UI minimized"
    $CheckBoxAutoupdate.AutoSize              = $false
    $CheckBoxAutoupdate.width                 = 160
    $CheckBoxAutoupdate.height                = 20
    $CheckBoxAutoupdate.location              = New-Object System.Drawing.Point(432,268)
    $CheckBoxAutoupdate.Font                  = 'Microsoft Sans Serif,10'
    $CheckBoxAutoupdate.Checked               =   $Config.StartGUIMinimized
